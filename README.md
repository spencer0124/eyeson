...
<table>
  <tr>
    <td><img src="./assets/award-grand-prize.svg" width="480" alt="최우수상 — 현대오토에버 대표이사상"/></td>
    <td><img src="./assets/award-sustained-ops.svg" width="480" alt="우수지속운영팀상 — 현대오토에버 배리어프리"/></td>
  </tr>
</table>
...
```
📸 촬영                    🧠 특징 추출                  🔍 유사도 검색               📱 결과
┌──────────┐  POST /search/  ┌──────────────┐  512-dim vector  ┌──────────────┐  Top 3   ┌──────────┐
│ iOS      │ ──────────────→ │ CLIP         │ ──────────────→ │ FAISS        │ ──────→ │ 작품 상세 │
│ Camera   │                 │ ViT-B/32     │                 │ IndexFlatL2  │         │ 화면 이동 │
└──────────┘                 └──────────────┘                 └──────────────┘         └──────────┘
                                                                     ↑
                                                              사전 계산된 등록
                                                              작품 벡터 DB
...
│                             │
│  POST /search/              │──→ CLIP ViT-B/32 + FAISS
│  POST /description/gpt-*   │──→ OpenAI GPT-4o
│  GET  /metadata/*           │──→ JSON Data
│  WS   /chat/ws/{museum}    │──→ Redis
│                             │
└──────────┬──────────────────┘
           │
    ┌──────┴──────┐
